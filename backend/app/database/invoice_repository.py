"""Invoice persistence and automatic Firm -> Matter -> Budget resolution."""
from __future__ import annotations
from datetime import date, datetime
from sqlalchemy.exc import IntegrityError
from app.database.database import SessionLocal
from app.models import Invoice, LineItem, Matter
from app.services.budget_management import resolve_or_create_from_invoice, apply_intake_snapshot

class InvoiceAlreadyExistsError(Exception):
    def __init__(self, invoice_no: str, matter_id: str | int, inv_changes: dict | None = None):
        self.invoice_no, self.matter_id, self.inv_changes = invoice_no, matter_id, inv_changes
        super().__init__(f"Invoice with invoice_no={invoice_no!r} already exists for matter_id={matter_id!r}")
class InvoiceNotFoundError(Exception):
    def __init__(self, invoice_id: int):
        self.invoice_id=invoice_id; super().__init__(f"No invoice with invoice_id={invoice_id!r}")

def _coerce_date(value):
    if not value or isinstance(value,date): return value
...
def get_or_create_matter(db, matter_no:str, matter_name:str|None=None, firm_name:str|None=None, firm_address:str|None=None):
    ...
    _, matter, _, _ = resolve_or_create_from_invoice(db,firm_name=firm_name,firm_address=firm_address,matter_no=matter_no,matter_name=matter_name)
    return matter

def invoice_exists(invoice_id:int)->bool:
    db=SessionLocal()
    try:return db.get(Invoice,invoice_id) is not None
    finally:db.close()

def invoice_exists_by_business_key(invoice_no:str,matter_id:str|int)->bool:
    db=SessionLocal()
    try:
        return db.query(Invoice).filter(Invoice.invoice_no==(invoice_no or "").strip(),Invoice.matter_id==matter_id).first() is not None
    finally:db.close()

def get_invoice(invoice_id:int):
    db=SessionLocal()
    try:return db.get(Invoice,invoice_id)
    finally:db.close()

def get_firm_id_for_matter(matter_id:int):
    db=SessionLocal()
    try:
        m=db.get(Matter,matter_id); return m.firm_id if m else None
    finally:db.close()

def insert_invoice_with_line_items(matter_id=None, firm_id=None, extracted:dict|None=None, confidence_score:float=0.0, status:str="submitted", *, firm_name:str|None=None, firm_address:str|None=None, matter_no:str|None=None, matter_name:str|None=None, user_id:int=-1)->int:
    """Persist invoice after automatically resolving/creating Firm, Matter and default Budget."""
    extracted=extracted or {}; db=SessionLocal()
    try:
        business_matter_no=(matter_no or extracted.get("matter_no") or "").strip()
        if not business_matter_no: raise ValueError("No matter identifier available for invoice.")
        firm,matter,budget,_=resolve_or_create_from_invoice(db,firm_name=firm_name or extracted.get("firm_name"),firm_address=firm_address or extracted.get("firm_address"),matter_no=business_matter_no,matter_name=matter_name or extracted.get("matter_name"),user_id=user_id)
        invoice_no=(extracted.get("invoice_no") or None)
        if isinstance(invoice_no,str): invoice_no=invoice_no.strip() or None
        if invoice_no and db.query(Invoice).filter(Invoice.invoice_no==invoice_no,Invoice.matter_id==matter.matter_id).first(): raise InvoiceAlreadyExistsError(invoice_no,matter.matter_id)
        invoice=Invoice(matter_id=matter.matter_id,firm_id=firm.firm_id,invoice_no=invoice_no,invoice_date=_coerce_date(extracted.get("invoice_date")),billing_period_start=_coerce_date(extracted.get("billing_period_start")),billing_period_end=_coerce_date(extracted.get("billing_period_end")),matter_name=extracted.get("matter_name") or matter.name,total_amount=float(extracted.get("total_amount") or 0),status=status,confidence_score=confidence_score)
        db.add(invoice); db.flush(); _apply_line_items(db,invoice.invoice_id,extracted.get("line_items")); db.flush()
        apply_intake_snapshot(db,invoice,budget,user_id=user_id); db.commit(); return invoice.invoice_id
    except IntegrityError as e:
        db.rollback(); raise InvoiceAlreadyExistsError((extracted or {}).get("invoice_no") or "UNKNOWN",matter_id or matter_no or "UNKNOWN") from e
    except Exception:
        db.rollback(); raise
    finally: db.close()

def update_invoice_with_line_items(invoice_id:int,matter_id:int,firm_id,extracted:dict,confidence_score:float,status:str)->int:
    db=SessionLocal()
    try:
        invoice=db.get(Invoice,invoice_id)
        if not invoice: raise InvoiceNotFoundError(invoice_id)
        invoice.invoice_no=(extracted.get("invoice_no") or invoice.invoice_no); invoice.invoice_date=_coerce_date(extracted.get("invoice_date")) or invoice.invoice_date; invoice.total_amount=float(extracted.get("total_amount") or invoice.total_amount); invoice.status=status; invoice.confidence_score=confidence_score
        db.query(LineItem).filter(LineItem.invoice_id==invoice_id).delete(); _apply_line_items(db,invoice_id,extracted.get("line_items")); db.commit(); return invoice_id
