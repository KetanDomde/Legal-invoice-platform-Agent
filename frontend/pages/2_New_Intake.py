    ...
    with st.expander("Advanced overrides (optional)"):
        ...
        with c1:
            matter_no_override = st.text_input(
                "Matter No. override", placeholder="e.g. MAT-771B",
                help="Only needed if the invoice has no extractable matter identifier, or to force a specific one.",
            )
        with c2:
            firm_name_override = st.text_input(
                "Firm name override", placeholder="e.g. Sample Outside Counsel LLP",
                help="Used to find-or-create the firm when this submission creates a brand-new matter.",
            )
            ...
            try:
                ...
                if e.status_code == 409:
                    ...
                    st.session_state["_last_intake_error"] = {
                        ...
                        "detail": f"{e.detail} Fill in **Matter No. override** above and resubmit.",
                        "inv_changes": None,
                    }
                else:
                    st.session_state["_last_intake_error"] = {
...
if error:
    with st.container(border=True):
        ...
        if error.get("inv_changes") is not None:
            st.markdown("###### What's different from the invoice already on file")
...
if result:
    ...
    with left:
        with st.container(border=True):
            ...
            billing_start, billing_end = extracted.get("billing_period_start"), extracted.get("billing_period_end")
            if billing_start or billing_end:
                kv_row("Billing Period", f"{billing_start or '—'} to {billing_end or '—'}")
            if extracted.get("matter_no") or extracted.get("matter_name"):
                kv_row("Matter (extracted)", " · ".join(filter(None, [extracted.get("matter_no"), extracted.get("matter_name")])))
    ...
    with right:
        with st.container(border=True):
            ...
            if result.get("is_duplicate"):
                ...
                notice("All checks passed so far — see the Review Queue if this needs a human decision.", success=True)
