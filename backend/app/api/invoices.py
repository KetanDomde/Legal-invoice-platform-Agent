...
async def submit_invoice(
    ...
    # get_firm_id_for_matter() returns None (not an error) for a
    # matter_id that doesn't exist yet — previously nothing checked for
    # that, so a bad matter_id crashed downstream in call_run_invoice_graph
    # with an unhandled AttributeError (None.firm_id) instead of a clean
    # 404. Checking here also avoids paying for a wasted extraction call
    # on a submission that can never succeed.
    if firm_id is None:
        raise HTTPException(
            status_code=404,
            detail=f"No matter with matter_id={matter_id!r}. Create the matter first.",
        )
    ...
    try:
        ...
        # Raised by persist_extracted_invoice() when the extracted
        # invoice_no collides with an existing (invoice_no, matter_id)
        # pair. This used to be an unhandled IntegrityError surfaced as a
        # bare 500 — see QA findings bug #5.
        raise HTTPException(
            status_code=409,
            detail=str(e),
        )
